...
Ganter \cite{orb,moonshine,orbifold,stringy}, among others.  The various 
cohomology theories each have as coefficients a ring of modular forms of a 
particular type---modular forms on $\SL_2(\BZ)$ with no condition at the cusp, 
$p$-adic modular forms, modular forms with level structure---or it is a closely 
related ring, e.g., a completion of the former at an ideal.  These cohomology 
...
\begin{proof}
 ...
 \begin{equation}
  \label{neq}
  \tilde{t}_{1,\,p} Q_1 = Q_1 \tilde{t}_{1,\,p} 
 \end{equation}
 We examine the coefficient of the term $Q_0 Q_1$ on each side, modulo $p$.  
 On the left-hand side, since 
 \[
  \tilde{t}_{1,\,p} Q_1 = \big( (p + 1) Q_0 - w_p Q_1 + c_2 Q_2 + \cdots 
  + c_p Q_p \big) Q_1 
 \]
 this coefficient is congruent to 1 modulo $p$.  
 We rewrite the right-hand side as 
 \[
   Q_1 \tilde{t}_{1,\,p} = Q_1 \big( (p + 1) Q_0 - w_p Q_1 + c_2 Q_2 + \cdots + c_p Q_p \big) 
 \]
 By means analogous to the proof of Proposition \ref{prop:Q}, we find 
 that the term $Q_0 Q_1$ has coefficient $w_2$ in the Adem relation for 
 $Q_1 Q_0$ (see Remark \ref{rmk:Adem}).  
 Thus, comparing the two sides of \eqref{neq} for $Q_0 Q_1$, we must have 
 \[
  1 \equiv w_2 + a \md p 
 \]
 where $a$ is the coefficient of $Q_0$ in $Q_1(-w_p)$ (after applying commutation relations).  
 Therefore, as modular functions, $w_2$ and $a$ satisfy the same congruence.  
 In particular, near the cusps, the choice of model and the geometric interpretation of $\A$ 
 guarantee \eqref{A0} and \eqref{Ai}, which imply that $p | w_i$ for $2 \leq i \leq p$.  
 Thus $a \equiv 1 \md p$ as a modular function near the cusps.  
 This means that $Q_1(-w_p) \not\equiv 0 \md p$ as an element in $\G$, and so 
 $-w_p \not\equiv 0 \md p$ in $E^0$.  
 The last congruence must then not hold when $w_p$ is viewed as a modular function.  
 This contradicts the divisibility exhibited above near the cusps.  
\end{proof}
\vskip .5cm
...
\bibitem[Baker et al.~2005]{Poonenetal}
...
\bibitem[Hill-Lawson2016]{logetaletmf}
