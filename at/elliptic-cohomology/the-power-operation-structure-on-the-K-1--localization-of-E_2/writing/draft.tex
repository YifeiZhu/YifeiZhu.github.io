...
\begin{defn}
\label{def:exotic}
 An {\em exotic morphism $\CP \to \CP'$ of moduli problems} is a rule 
...
In particular, the rule of an exotic morphism $\CP \to \CP'$ sends an 
isomorphism 
\begin{center}
\begin{tikzpicture}
        \node (LT) at (0, 2) {$E_1$}; 
...
classes the exotic morphism $\CP \to \CP'$ then induces a morphism 
$\CM(\CP) \to \CM(\CP')$ of schemes.  

\begin{rmk}
 A morphism $\CP \to \CP'$ (see Definition \ref{def:mp}) is an exotic 
...
\label{prop:Q}
 ...
 \begin{enumerate}[(i)]
  ...
  \begin{equation*}
   Q_0(x) \equiv x^3 \md 3.  \qquad \qquad \qquad \qquad \qquad \qquad 
